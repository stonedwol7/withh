...
import { Clock, MapPin, LogOut, User } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (partner.verification_status === 'pending' || partner.verification_status === 'rejected') {
          if (!cancelled) router.replace('/partner/kyc')
          return
        }

        if (!cancelled) setPartnerName(partner.name || 'Partner')
