  ...
  return (
    <aside className="w-64 bg-card border-r border-border h-screen sticky top-0 overflow-y-auto flex flex-col" role="navigation" aria-label="Operations sidebar">
      <div className="p-5 border-b border-border">
        <span className="text-xl font-bold text-foreground">WITHH</span>
        <span className="text-xs text-amber ml-2 font-medium">Ops</span>
      </div>
      ...
      <div className="p-3 border-t border-border">
        <button
          onClick={() => { doLogout(); router.push('/login') }}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-destructive hover:bg-destructive/5 transition-colors"
        >
